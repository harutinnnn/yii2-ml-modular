...
<div class="post-form">
    ...
    <div class="card card-primary">
        <div class="card-body">
            <?= $form->field($model, 'status')->dropDownList(Events::statusOptions()) ?>
    ...
    <div class="card card-outline card-primary">
            ...
            <div class="tab-content">
                <?php foreach ($languages as $index => $language): ?>
                    ...
                    >
                        ...
                        <?= $form->field($model, "translations[{$language->code}][place]")
                                ->label("Place ({$language->name})")
                                ->textInput([
                                        'maxlength' => true,
                                        'placeholder' => "Place in {$language->name}",
                                ]) ?>

                        <div class="row">
                            <div class="col-md-6">
                                <?= $form->field($model, "translations[{$language->code}][organiser]")
                                        ->label("Organiser ({$language->name})")
                                        ->textInput([
                                                'maxlength' => true,
                                                'placeholder' => "Organiser in {$language->name}",
                                        ]) ?>
                            </div>
                            <div class="col-md-6">
                                <?= $form->field($model, "translations[{$language->code}][contact]")
                                        ->label("Contact ({$language->name})")
                                        ->textInput([
                                                'maxlength' => true,
                                                'placeholder' => "Contact in {$language->name}",
                                        ]) ?>
                            </div>
                        </div>
        ...
        <div class="card-footer">
            <?= Html::submitButton($model->events === null ? 'Create' : 'Update', ['class' => 'btn btn-primary']) ?>
